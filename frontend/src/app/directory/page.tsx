...
export default function Directory() {
  ...
  const [activeTab, setActiveTab] = useState<'md' | 'lion' | 'district' | 'clubs'>('md');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const l = await db.getLionCabinet();
      setDistricts(d);
      setClubs(c);
      setMdCabinet(m);
      setLionCabinet(l);
  ...
  const filteredLionCabinet = lionCabinet.filter(p =>
    p.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    p.role.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full">
      ...
      {/* DIRECTORY TABS */}
      <div className="flex overflow-x-auto border-b border-white/10 text-[10px] sm:text-xs tracking-widest uppercase font-semibold text-silver-primary mb-10 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide">
        <button
          onClick={() => setActiveTab('md')}
          className={`px-3 sm:px-6 py-3 sm:py-4 border-b-2 transition-all whitespace-nowrap ${activeTab === 'md' ? 'border-gold-primary text-gold-light font-bold bg-white/5' : 'border-transparent hover:text-white'
            }`}
        >
          MD Cabinet
        </button>
        <button
          ...
          className={`px-3 sm:px-6 py-3 sm:py-4 border-b-2 transition-all whitespace-nowrap ${activeTab === 'lion' ? 'border-gold-primary text-gold-light font-bold bg-white/5' : 'border-transparent hover:text-white'
            }`}
        >
          Lion Cabinet
        </button>
        <button
          ...
          className={`px-3 sm:px-6 py-3 sm:py-4 border-b-2 transition-all whitespace-nowrap ${activeTab === 'district' ? 'border-gold-primary text-gold-light font-bold bg-white/5' : 'border-transparent hover:text-white'
            }`}
        >
          District Cabinets
        </button>
        <button
          ...
          className={`px-3 sm:px-6 py-3 sm:py-4 border-b-2 transition-all whitespace-nowrap ${activeTab === 'clubs' ? 'border-gold-primary text-gold-light font-bold bg-white/5' : 'border-transparent hover:text-white'
            }`}
        >
          Club Directory
        </button>
              ...
              {filteredMDCabinet.map((profile) => (
                <div key={profile.id} className="glass-panel rounded-3xl overflow-hidden border border-white/5 hover:border-gold-primary/20 transition-all group flex flex-col justify-between min-h-[360px] relative">
                  ...
                  <div className="relative w-full aspect-square bg-bg-deep-space overflow-hidden flex items-center justify-center border-b border-white/5">
                    {profile.photo ? (
                      <img src={toDirectImageUrl(profile.photo)} alt={profile.name} className="w-full h-full object-cover group-hover:scale-102 transition-transform duration-500" />
                    ) : (
      ...
      {/* 1B. TAB CONTENT: LION CABINET */}
      {activeTab === 'lion' && (
        <div>
          {filteredLionCabinet.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
              {filteredLionCabinet.map((profile) => (
                <div key={profile.id} className="glass-panel rounded-3xl overflow-hidden border border-white/5 hover:border-gold-primary/20 transition-all group flex flex-col justify-between min-h-[360px] relative">
                  <div className="relative w-full aspect-square bg-bg-deep-space overflow-hidden flex items-center justify-center border-b border-white/5">
                    {profile.photo ? (
                      <img src={toDirectImageUrl(profile.photo)} alt={profile.name} className="w-full h-full object-cover group-hover:scale-102 transition-transform duration-500" />
            ...
            <div className="text-center py-20 text-xs text-silver-dark">No Lion Cabinet members found matching your search.</div>
          )}
        </div>
      )}
                          ...
                          <div key={leader.id} className="p-4 bg-white/5 border border-white/5 rounded-xl hover:border-gold-primary/20 transition-all flex flex-col justify-between">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-lg overflow-hidden bg-bg-deep-space border border-white/10 flex items-center justify-center shrink-0">
                                {leader.photo ? (
                                  <img src={toDirectImageUrl(leader.photo)} alt={leader.name} className="w-full h-full object-cover" />
                                ) : (

...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'stats' | 'directory' | 'projects' | 'media' | 'events' | 'site' | 'cabinet'>('stats');
  ...
  const loadData = async () => {
    const [p, c, d, e, m, s, cnt, tk] = await Promise.all([
      db.getProjects(),
      db.getClubs(),
      db.getDistricts(),
      db.getEvents(),
        ...
        {activeTab === 'events' && (
          <div className="space-y-8">
            ...
            <form onSubmit={async (e) => {
              ...
              await db.addEvent({ title: evtTitle, description: evtDesc, date: evtDate, location: evtLocation, district: evtDistrict, status: evtStatus, poster: '', registrationLink: '', organizingTeam: '' });
              setEvtTitle(''); setEvtDesc(''); setEvtDate(''); setEvtLocation('');
              loadData();
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-[8px] uppercase tracking-wider text-silver-dark mb-1">District</label>
                  <select value={evtDistrict} onChange={e => setEvtDistrict(e.target.value)} className="w-full px-2.5 py-2 bg-bg-deep-space border border-white/10 rounded-lg text-xs cursor-pointer">
        ...
        {activeTab === 'cabinet' && (
          <div className="space-y-8">
            ...
            <form onSubmit={async (e) => {
              ...
              await db.addCabinetMember({ name: cabName, role: cabRole, district: cabDistrict, club: cabClub, email: cabEmail, phone: cabPhone });
              setCabName(''); setCabRole(''); setCabClub(''); setCabEmail(''); setCabPhone('');
              loadData();
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={cabClub} onChange={e => setCabClub(e.target.value)} placeholder="e.g. Leo Club of Satva" className="w-full px-2.5 py-2 bg-bg-deep-space border border-white/10 rounded-lg focus:outline-none focus:border-gold-primary text-xs" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>

            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.path}
                  className={`relative text-xs tracking-widest uppercase font-medium transition-all duration-300 py-1 hover:text-gold-light ${isActive ? 'text-gold-primary font-semibold' : 'text-silver-light/70'
                    }`}
                >
                  {link.name}
                  {isActive && (
